...
**智能 Dockerfile 审查、分析与自动修复 CLI 工具**
...
**自研差异化亮点**：灵感来源于 [dockerfile-roast](https://github.com/immanuwell/dockerfile-roast)（Rust 写的 Dockerfile linter），但做了全面升级——Python 实现（零 Rust 工具链依赖）、18 条规则覆盖更全、7 条自动修复（原项目无修复能力）、0-100 评分系统、CI 友好的 JSON 输出与退出码、dry-run 安全预览模式。
...
**What makes it different**: Inspired by [dockerfile-roast](https://github.com/immanuwell/dockerfile-roast) (Rust Dockerfile linter), but comprehensively upgraded — pure Python (zero Rust toolchain), 18 rules, 7 auto-fixes (original has none), 0-100 scoring, CI-friendly JSON output + exit codes, dry-run preview mode.
...
Deduction: Error -15 / Warning -5 / Info -2 / Style -1
...
Returns structured JSON with file, score, counts, and full issue details.
...
- [ ] LLM-powered smart fix suggestions
...
- **オピニオンated** — ルールは単なる提案ではなく、プロフェッショナルな判断
- **セキュリティファースト** — 秘密鍵の露出、rootユーザー、特権ポートはErrorとしてマーク
- **自動修正** — 報告するだけでなく、修正する
- **CIフレンドリー** — JSON出力 + 終了コードでシームレスなパイプライン統合
...
- [ ] イメージサイズ推定
- [ ] スコア傾向トラッキング
- [ ] Docker Hubダイジェスト自動検索
- [ ] ベストプラクティスレポート生成（PDF/HTML）
